package com.wangan.usercenter.service.impl;
import java.util.Date;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.wangan.usercenter.common.ErrorCode;
import com.wangan.usercenter.constant.UserConstant;
...
import com.wangan.usercenter.mapper.UserMapper;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService{
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword,String planetCode) {
        //1、校验（commons-lang3使用工具可以判断多个字符是否为空）
        ...
        boolean saveResult = this.save(user);//防止拆箱null值影响错误；
        if (!saveResult) {
            return -1;
        }
        return user.getId();
    ...
    public User userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        Matcher matcher = Pattern.compile(validPattern).matcher(userAccount);
        if (matcher.find()) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "账号不规范");
        }
        //加密
